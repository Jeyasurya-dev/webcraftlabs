...
def create_enquiry(form: dict) -> int:
    ...
    return execute(
        """INSERT INTO enquiries (name, email, phone, company, service, budget_range, message)
           VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (data["name"], data["email"], data["phone"], data["company"],
         data["service"], data["budget_range"], data["message"]),
    )


def list_enquiries(status: str = None):
    if status:
        return query_all("SELECT * FROM enquiries WHERE status = ? ORDER BY created_at DESC", (status,))
    return query_all("SELECT * FROM enquiries ORDER BY created_at DESC")


def get_enquiry(enquiry_id: int):
    return query_one("SELECT * FROM enquiries WHERE id = ?", (enquiry_id,))


def update_enquiry_status(enquiry_id: int, status: str):
    execute("UPDATE enquiries SET status = ?, updated_at = datetime('now') WHERE id = ?", (status, enquiry_id))
